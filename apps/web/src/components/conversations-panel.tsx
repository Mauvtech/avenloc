...
// Cœur de la messagerie (liste + fil), partagé entre /conversations et
// l'onglet « Messagerie » du dashboard hôte — la conversation ne s'ouvre
// jamais avant la réservation, voir handleBookingConfirm côté backend.
export default function ConversationsPanel() {
  ...
  useEffect(() => {
    Promise.all([api.auth.me().catch(() => null), api.conversations.list().catch(() => [])]).then(
      ([me, cs]) => {
        setMeId(me?.id ?? null);
        setConversations(cs);
  ...
  async function selectConversation(conv: Conversation) {
    ...
    setConversations((cs) => (cs ? cs.map((c) => (c.id === conv.id ? { ...c, unreadCount: 0 } : c)) : cs));
  }
  ...
  return (
    <div className="grid h-[70vh] min-h-[520px] grid-cols-1 gap-4 sm:grid-cols-3">
      {/* Liste */}
      <div className={`card col-span-1 overflow-y-auto ${active ? 'hidden sm:block' : 'block'}`}>
        {conversations.map((c) => (
          <button
            key={c.id}
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-2">
                ...
                {(c.unreadCount ?? 0) > 0 && <span className="h-2 w-2 flex-none rounded-full bg-brand" />}
              </div>
              <div className="truncate text-xs font-semibold text-brand-fg">{c.listingTitle}</div>
              {c.lastMessage && <div className="truncate text-xs text-muted">{c.lastMessage.content}</div>}
            </div>
          </button>
        ))}
      </div>
      ...
      {/* Fil */}
      <div className={`card col-span-1 flex-col sm:col-span-2 sm:flex ${active ? 'flex' : 'hidden'}`}>
        {active ? (
          <>
              ...
              {messages.map((m) => {
                ...
                return (
                  <div key={m.id} className={`flex items-end gap-2 ${mine ? 'flex-row-reverse' : ''}`}>
                    {!mine && <Avatar src={m.senderAvatarUrl} name={m.senderName || partyName(active)} size={26} />}
                    <div className="max-w-[75%]">
                      {!mine && m.senderName && (
                        <div className="mb-0.5 text-[11px] font-semibold text-muted">{m.senderName}</div>
                      )}
                      <div
                      ...
                      <div className={`mt-0.5 text-[10px] text-muted ${mine ? 'text-right' : ''}`}>
                        {msgTime(m.createdAt)}
                      </div>
                    </div>
                  </div>
